'''
In this file, we simulate convergence of different policies with real-world initial distribution. The codes are vectorized. We choose f/g functions to be (segment) linear.
'''
...
'''
In the next part we read the income from the SIPP dataset and use it as the representation of the welfare level. 
We keep the distribution of the population income with shift and normalization. We use 1000 dollars as a unit, and 200 individuals as a population unit in simulation. 
The 200-individual group corresponds to one individual in our dynamical model. Another modeling assumption is distributing several budgets at one time step.
'''
...
homo_expected_zeta = (homo_return_ub - (agent_num-1) * homo_decay_lb) / agent_num
print(homo_expected_zeta)
homo_expected_zeta_greedy = (homo_return_ub - (agent_num-1) * homo_decay_ub) / agent_num
print("expected greedy rbar:", homo_expected_zeta_greedy)

'''
after reading the real distributions, we generate return/decay alphas and return/decay betas for each individual
'''
...
# plt.xlim(0, 1000)
plt.ylim(330, 385)
plt.legend(loc='center right', bbox_to_anchor=(0.95, 0.47), fontsize=12)
...
''' 
Plot gini coefficient comparison
'''
if if_gini:
    plt.figure()
    plt.xlabel("Time step t", fontsize=18)
    plt.ylabel("Gini index", fontsize=18)
    xs = np.array([i for i in range(max_time_horizon)])
    if "maximin" or "all" in policies:
        with open(homo_hetero_signature+"maximin_gini_mean_discrete.pk", 'rb') as file:
            maximin_gini_mean_discrete = pickle.load(file)
        with open(homo_hetero_signature+"maximin_gini_std_discrete.pk", 'rb') as file:
            maximin_gini_std_discrete = pickle.load(file)

        plt.plot(maximin_gini_mean_discrete, label="Maximin")
        plt.fill_between(xs, maximin_gini_mean_discrete-maximin_gini_std_discrete, maximin_gini_mean_discrete+maximin_gini_std_discrete, alpha=0.4)

    if "fg_maximax" or "all" in policies:

        with open(homo_hetero_signature+"fg_maximax_gini_mean_discrete.pk", 'rb') as file:
            fg_maximax_gini_mean_discrete = pickle.load(file)
        with open(homo_hetero_signature+"fg_maximax_gini_std_discrete.pk", 'rb') as file:
            fg_maximax_gini_std_discrete = pickle.load(file)

        plt.plot(fg_maximax_gini_mean_discrete, label = "fg-maximax")
        plt.fill_between(xs, fg_maximax_gini_mean_discrete-fg_maximax_gini_std_discrete, fg_maximax_gini_mean_discrete+fg_maximax_gini_std_discrete, alpha=0.4)

    if "f_maximax" or "all" in policies:

        with open(homo_hetero_signature+"f_maximax_gini_mean_discrete.pk", 'rb') as file:
            f_maximax_gini_mean_discrete = pickle.load(file)
        with open(homo_hetero_signature+"f_maximax_gini_std_discrete.pk", 'rb') as file:
            f_maximax_gini_std_discrete = pickle.load(file)

        plt.plot(f_maximax_gini_mean_discrete, label = "f-maximax")
        plt.fill_between(xs, f_maximax_gini_mean_discrete-f_maximax_gini_std_discrete, f_maximax_gini_mean_discrete+f_maximax_gini_std_discrete, alpha=0.4)

    if "maximax" or "all" in policies:
        with open(homo_hetero_signature+"maximax_gini_mean_discrete.pk", 'rb') as file:
            maximax_gini_mean_discrete = pickle.load(file)
        with open(homo_hetero_signature+"maximax_gini_std_discrete.pk", 'rb') as file:
            maximax_gini_std_discrete = pickle.load(file)

        plt.plot(maximax_gini_mean_discrete, label="Maximax")
        plt.fill_between(xs, maximax_gini_mean_discrete-maximax_gini_std_discrete, maximax_gini_mean_discrete+maximax_gini_std_discrete, alpha=0.4)



    if "random" or "all" in policies:
        with open(homo_hetero_signature+"random_gini_mean_discrete.pk", 'rb') as file:
            random_gini_mean_discrete = pickle.load(file)
        with open(homo_hetero_signature+"random_gini_std_discrete.pk", 'rb') as file:
            random_gini_std_discrete = pickle.load(file)
    
        plt.plot(random_gini_mean_discrete, label="Random")
        plt.fill_between(xs, random_gini_mean_discrete-random_gini_std_discrete, random_gini_mean_discrete+random_gini_std_discrete, alpha=0.4)


    # plt.xlim(0, 200)
    # plt.ylim(-0.5, 3)
    # plt.hlines(xmin=0, xmax=max_time_horizon, y=0, linestyles='--')
    plt.legend(loc='center right', bbox_to_anchor=(0.95, 0.5), fontsize=12)
    # plt.legend(loc='upper right')

    plt.savefig(homo_hetero_signature+"gini_compare_discrete.pdf", bbox_inches='tight')
    plt.show()